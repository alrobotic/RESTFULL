...
public class ConfigController {
    ...
    public static Route getConstantMensaje = (Request request, Response response) -> {
        try {
            response.body(dataToJson(configHandler.getMensaje()));
            response.status(200);
    ...
    public static Route setConstantMensaje = (Request request, Response response) -> {
        try {
            configHandler.setMensaje(request.params("mensaje"));
            response.body("{\"newValueForConstantS\": \"" + request.params("mensaje") + "\"}");
            response.status(200);
